 ...
 * 
 * This class represents a text sample to be parsed.<br/>
 ...
 * the accessor functions.<br/>
 * This class is intended to be used by the Text_LanguageDetect class, not end-users.<br/>
...
class Text_LanguageDetect_Parser extends Text_LanguageDetect {
    ...
    function analyze() {
        ...
        while ($byte_counter < $len) {
            ...
            if ($this->_compile_unicode) {
                if ($this->_unicode_skip_symbols && strlen($char) == 1 && ($char < 'A' || $char > 'z' || ($char > 'Z' && $char < 'a')) && $char != "'") {  // does not skip the apostrophe
