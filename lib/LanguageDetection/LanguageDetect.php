...
require_once 'lib/LanguageDetectException.php';
require_once 'lib/Parser.php';
require_once 'lib/ISO639.php';

/**
 * 
 * @class Text_LanguageDetect
 * 
 * Detects the language of a given piece of text.<br/>
 ...
 * accompanied this class definition in order to be instantiated.<br/>
 * Implements a version of a technique originally proposed by Cavnar & Trenkle
 * (1994): "N-Gram-Based Text Categorization".<br/>
 * http://pear.php.net/package/Text_LanguageDetect/<br/>
 * http://langdetect.blogspot.com/<br/>
 *
 * @package   Text_LanguageDetect
 * @author    Nicholas Pisarro <infinityminusnine+pear@gmail.com><br/>
 * @copyright 2005-2006 Nicholas Pisarro<br/>
...
class Text_LanguageDetect {
     ...
     * $this->_data_dir will be ignored
     *
     * @var string  $_unicode_db_filename
     */
     ...
     *
     * @var bool    $_use_unicode_narrowing
     */
    ...
    function _readdb($fname) {
        ...
        if (!file_exists($fname)) {
            ...
            'Language database does not exist: ' . $fname, LanguageDetectException::DB_NOT_FOUND
            );
        } elseif (!is_readable($fname)) {
            throw new LanguageDetectException(
            'Language database is not readable: ' . $fname, LanguageDetectException::DB_NOT_READABLE
            );
        }
    ...
    function _checkTrigram($trigram) {
        if (!is_array($trigram)) {
            if (ini_get('magic_quotes_runtime')) {
                throw new LanguageDetectException(
                'Error loading database. Try turning magic_quotes_runtime off.', LanguageDetectException::MAGIC_QUOTES
                );
            }
            throw new LanguageDetectException(
            'Language database is not an array.', LanguageDetectException::DB_NOT_ARRAY
            );
        } elseif (empty($trigram)) {
            throw new LanguageDetectException(
            'Language database has no elements.', LanguageDetectException::DB_EMPTY
            );
        }
    }
    ...
    public function languageExists($lang) {
        ...
        if (is_string($lang)) {
            ...
            throw new LanguageDetectException(
            'Unsupported parameter type passed to languageExists()', LanguageDetectException::PARAM_TYPE
            );
        }
    }
    ...
    function getLanguages() {
        return $this->_convertToNameMode(array_keys($this->_lang_db));
    }
     ...
     * @return int the sum of the differences between the ranks of the two trigram sets
     */
    ...
    public function detect($sample, $limit = 0) {
        ...
        if ($this->_use_unicode_narrowing) {
            ...
            if (is_array($blocks)) {
                ...
                throw new LanguageDetectException(
                'Error during block detection', LanguageDetectException::BLOCK_DETECTION
                );
            }
            ...
            foreach ($present_blocks as $blockname) {
                if (isset($this->_unicode_map[$blockname])) {
                    ...
                    $possible_langs = array_merge(
                            $possible_langs, array_keys($this->_unicode_map[$blockname])
                    );
                }
            }
            ...
            $possible_langs = array_intersect(
                    array_keys($this->_lang_db), array_unique($possible_langs)
            );
        ...
        foreach ($possible_langs as $lang) {
            $scores[$lang] = $this->_normalize_score(
                    $this->_distance($this->_lang_db[$lang], $trigram_freqs), $trigram_count
            );
        }
    ...
    public function detectSimple($sample) {
        ...
        if (!is_array($scores) || empty($scores) || current($scores) == $this->_max_score
        ) {
            return null;
        } else {
            return key($scores);
    ...
    public function unicodeBlockName($unicode) {
        if (is_string($unicode)) {
            ...
            if (self::utf8strlen($unicode) > 1) {
                throw new LanguageDetectException(
                'Pass a single char only to this method', LanguageDetectException::PARAM_TYPE
                );
            }
            $unicode = $this->_utf8char2unicode($unicode);
        } elseif (!is_int($unicode)) {
            throw new LanguageDetectException(
            'Input must be of type string or int.', LanguageDetectException::PARAM_TYPE
            );
        }
        ...
        if ($lang1 != null) {
            ...
            if ($lang2 != null) {
                ...
                return $this->_normalize_score(
                                $this->_distance(
                                        $this->_lang_db[$lang1], $this->_lang_db[$lang2]
                                )
                );
            } else {
                ...
                foreach (array_keys($this->_lang_db) as $lang2) {
                    ...
                    if ($lang1 != $lang2) {
                        ...
                        if (isset($return_arr[$lang2][$lang1])) {
                            ...
                            // calculate
                            $return_arr[$lang1][$lang2] = $this->_normalize_score(
                                    $this->_distance(
                                            $this->_lang_db[$lang1], $this->_lang_db[$lang2]
                                    )
                            );
     ...
     * @deprecated  Method will eventually be removed and placed into the model generation class.
     */
    function clusterLanguages() {
        ...
        foreach ($langs as $lang) {
            if (!isset($this->_lang_db[$lang])) {
                throw new LanguageDetectException(
                "missing $lang!", LanguageDetectException::UNKNOWN_LANGUAGE
                );
            }
        }
        ...
        while (count($langs) > 2 && $i++ < 200) {
            ...
            foreach ($langs as $lang1) {
                foreach ($langs as $lang2) {
                    if ($lang1 != $lang2 && $arr[$lang1][$lang2] > $highest_score
                    ) {
            ...
            if (!$highest_key1) {
                ...
                throw new LanguageDetectException(
                "no highest key? (step: $i)", LanguageDetectException::NO_HIGHEST_KEY
                );
            }
    ...
    public function clusteredSearch($str) {
        ...
        foreach ($dendogram_start as $lang) {
            ...
            $scores[$lang] = $this->_normalize_score(
                    $this->_distance($this->_lang_db[$lang_key], $sample_result), $sample_count
            );
        ...
        if ($ord <= 127) {
            ...
            if ($special_convert) {
                ...
                if ($ord == 195) {
                    ...
                    if ($nextord_adj >= 192 && $nextord_adj <= 222 && $nextord_adj != 215
                    ) {
